...
export default function Allelectronics() {
  ...
  // Pagination logic
  const totalPages = Math.ceil(electronicsproducts.length / ITEMS_PER_PAGE);

  const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
  const currentProducts = electronicsproducts.slice(
    startIndex,
    ...
    <div className="min-h-screen bg-gray-100">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mb-6 flex items-center justify-between bg-[#E61601] px-4 py-2 text-[#FEFAFA] md:px-6">
          ...
          <p className="text-sm">{electronicsproducts.length} items</p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 md:grid-cols-4 lg:grid-cols-5">
          {currentProducts.map((item) => (
            <div
              key={item.id}
              onClick={() => navigate(`/electronicsproductdetails/${item.id}`)}
