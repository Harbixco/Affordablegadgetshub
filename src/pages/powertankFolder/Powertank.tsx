...
import { solartankproducts } from "../../DummyData/data";
...
export default function Powerbank() {
  ...
  useEffect(() => {
    const handleResize = () => {
      if (window.innerWidth >= 768) {
        setItemsToShow(10); // md and above
      } else {
  ...
  return (
    <>
      ...
      >
        <h2 className="text-lg font-semibold md:text-xl">Solar Products</h2>

        <button
          onClick={() => navigate("/all-solar-tanks")}
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4 lg:grid-cols-5">
        {solartankproducts.slice(0, itemsToShow).map((item) => (
          <div
            key={item.id}
