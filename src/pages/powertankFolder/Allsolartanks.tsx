...
export default function Allsolartanks() {
  ...
  // Pagination logic
  const totalPages = Math.ceil(solartankproducts.length / ITEMS_PER_PAGE);

  const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
  const currentProducts = solartankproducts.slice(
    startIndex,
    ...
    <div className="min-h-screen bg-gray-100">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mb-6 flex items-center justify-between bg-[#E61601] px-4 py-2 text-[#FEFAFA] md:px-6">
          <h1 className="text-xl font-bold md:text-3xl">Solar Products</h1>
          <p className="text-sm">{solartankproducts.length} items</p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 md:grid-cols-4 lg:grid-cols-5">
          {currentProducts.map((item) => (
            <div
              key={item.id}
              onClick={() => navigate(`/solarproductdetails/${item.id}`)}
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 py-5 md:mt-8 md:py-10">
          ...
          <button
            onClick={() => {
              window.scrollTo({ top: 0, behavior: "smooth" }); // ✅ IMPORTANT
